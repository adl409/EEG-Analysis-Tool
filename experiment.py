...
def getParticipantsExperiments(particpantNumber, interval):
    ...
    for i in range(60):
        ...
        if(temp.df.empty == False):
            ...
            temp.data[0] = np.array(temp.data[0])
...
def getAllData(interval):
    ...
    for i in range(30):
        participantData = getParticipantsExperiments(i, interval)
        if len(participantData) > 0:
            print("Data for participant:", i, "has been collected..... \n")
            allData.append(participantData)
    

    # Returns list in the form:
    #
    # alldata = [Participant1, Participant2, ...]
    #
    #                    EXPERIMENT 1       EXPERIMENT 2
    # Participant1 = [[npArray, Result], [npArray, Result], ... ]
    #
